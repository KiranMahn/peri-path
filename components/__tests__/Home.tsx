import React from 'react';
import { render, fireEvent, waitFor, act } from '@testing-library/react-native';
import AsyncStorage from '@react-native-async-storage/async-storage';
import Home from '../../app/(tabs)/home';
import { SettingsContext } from '../../app/settings-context';

jest.mock('@react-native-async-storage/async-storage', () => ({
    getItem: jest.fn(),
    setItem: jest.fn(),
}));

jest.mock('../../app/widgets/calendar/two-week', () => () => <></>);
jest.mock('../../app/widgets/analysis/last-period', () => () => <></>);
jest.mock('../../app/widgets/analysis/most-common-symptom', () => () => <></>);
jest.mock('../../app/widgets/analysis/symptom-chart', () => () => <></>);
jest.mock('../../app/widgets/analysis/period-chart', () => () => <></>);
jest.mock('../../app/widgets/nav', () => () => <></>);

describe('Home Component', () => {

    const renderComponent = (settings: { largeText: boolean; highContrast: boolean; largeIcons: boolean; }) => {
        return render(
          <SettingsContext.Provider value={{ settings, saveSettings: jest.fn() }}>
            <Home />
          </SettingsContext.Provider>
        );
      };
    

    it('renders correctly', async () => {
        const { getByText } = await renderComponent({
            largeText: false,
            highContrast: false,
            largeIcons: false,
        });
        expect(getByText('Symptoms')).toBeTruthy();
        expect(getByText('Period')).toBeTruthy();
    });

    // it('retrieves and sets user data from AsyncStorage', async () => {
    //     const mockUser = { username: 'testuser' };
    //     const mockUsers = [{ username: 'testuser', data: 'someData' }];
    
    //     jest.spyOn(AsyncStorage, 'getItem').mockImplementation(async (key) => {
    //         if (key === 'user') return JSON.stringify(mockUser);
    //         if (key === 'users') return JSON.stringify(mockUsers);
    //         return null;
    //     });
    
    //     await act(async () => {
    //         // const { getByText } = await renderComponent({
    //         //     largeText: false,
    //         //     highContrast: false,
    //         //     largeIcons: false,
    //         // });
    
    //         // Wait for AsyncStorage calls to complete
    //         await waitFor(() => {
    //             expect(AsyncStorage.getItem).toHaveBeenCalledWith('user');
    //             expect(AsyncStorage.getItem).toHaveBeenCalledWith('users');
    //         });
    //     });
    // });
    

    // it('toggles between symptom and period charts', () => {
    //     const { getByText, queryByText } = render(<Home />);
    //     const symptomsButton = getByText('Symptoms');
    //     const periodButton = getByText('Period');

    //     // Default is 'symptoms'
    //     expect(queryByText('Symptoms')).toBeTruthy();

    //     // Switch to 'period'
    //     fireEvent.press(periodButton);
    //     expect(queryByText('Period')).toBeTruthy();

    //     // Switch back to 'symptoms'
    //     fireEvent.press(symptomsButton);
    //     expect(queryByText('Symptoms')).toBeTruthy();
    // });

    // it('triggers refresh on pull-to-refresh', async () => {
    //     const { getByTestId } = render(<Home />);
    //     const scrollView = getByTestId('scrollView');

    //     await act(async () => {
    //         fireEvent.scroll(scrollView, { nativeEvent: { contentOffset: { y: -100 } } });
    //     });

    //     expect(AsyncStorage.getItem).toHaveBeenCalled();
    // });
});
